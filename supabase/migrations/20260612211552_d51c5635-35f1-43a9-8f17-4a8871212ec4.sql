...
BEGIN
  ...
  IF _start IS NULL OR _ship.stealing_ends_at IS NULL THEN
    _ratio := 0;
  ELSE
  ...
  IF _scaled > 0 THEN
    _remaining := _scaled;
    SELECT sc.fish_pool INTO _pool FROM public.ships_owned so JOIN public.ship_catalog sc ON sc.code = COALESCE(so.catalog_code, 'ship-lvl-' || COALESCE(so.template_id, 1)) WHERE so.id = _target_ship_id AND so.user_id = _target_user_id LIMIT 1;
    IF _pool IS NULL THEN _pool := '[]'::jsonb; END IF;

    FOR _row IN
      WITH pool_ids AS (SELECT jsonb_array_elements_text(_pool) AS fid)
      SELECT fs.id, fs.fish_id, fs.base_value, GREATEST(0, COALESCE(fs.quantity, 1))::int AS quantity FROM public.fish_stock fs
    ...
    LOOP
      ...
      INSERT INTO public.fish_stock(user_id, fish_id, ship_id, caught_at, base_value, quantity) VALUES (_ship.user_id, _row.fish_id, _attacker_ship_id, now(), _row.base_value, _take);
      INSERT INTO public.fish_caught(user_id, fish_id, quantity, total_caught, updated_at) VALUES (_ship.user_id, _row.fish_id, _take, _take, now())
      ...
      INSERT INTO public.competition_catches(user_id, fish_id, caught_at, qty) VALUES (_ship.user_id, _row.fish_id, now(), _take);
      _moved := _moved + _take;
    ...
    IF _remaining > 0 AND _target_ship.id IS NOT NULL AND _target_ship.fishing_started_at IS NOT NULL THEN
      ...
      IF _pool_len > 0 THEN
        ...
        ELSE
          SELECT p.value INTO _generated_fish_id FROM jsonb_array_elements_text(_target_pool) WITH ORDINALITY AS p(value, ord)
          WHERE p.ord = (1 + (abs(hashtextextended(_target_ship_id::text || ':' || _target_ship.fishing_started_at::text, 91003)) % _pool_len)) LIMIT 1;
        END IF;
        ...
        IF _take > 0 THEN
          ...
          INSERT INTO public.fish_stock(user_id, fish_id, ship_id, caught_at, base_value, quantity) VALUES (_ship.user_id, _generated_fish_id, _attacker_ship_id, now(), COALESCE(_unit_value, 0), _take);
          INSERT INTO public.fish_caught(user_id, fish_id, quantity, total_caught, updated_at) VALUES (_ship.user_id, _generated_fish_id, _take, _take, now())
          ON CONFLICT (user_id, fish_id) DO UPDATE SET quantity = public.fish_caught.quantity + EXCLUDED.quantity, total_caught = public.fish_caught.total_caught + EXCLUDED.total_caught, updated_at = now();
          INSERT INTO public.competition_catches(user_id, fish_id, caught_at, qty) VALUES (_ship.user_id, _generated_fish_id, now(), _take);
          _moved := _moved + _take;
