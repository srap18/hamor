...
BEGIN
  ...
  -- Threshold: only fish strictly more valuable than the attacker's best
  SELECT COALESCE(MAX(base_value), 0) INTO _my_max
    FROM public.fish_stock WHERE user_id = _attacker;

  WITH picked AS (
    SELECT id, base_value FROM public.fish_stock
    WHERE user_id = _defender_id
      AND base_value > _my_max
