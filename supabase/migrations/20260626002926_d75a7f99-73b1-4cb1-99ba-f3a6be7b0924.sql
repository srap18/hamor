...
DECLARE
  ...
  _ships_processed int := 0; _total_cycles int := 0;
...
BEGIN
  ...
  -- Cancel attacks against this player while protected (kept from previous behavior)
  ...
  -- Only act on ships that are CURRENTLY fishing and whose timer has hit zero.
  -- Do NOT auto-relaunch ships the player intentionally stopped.
  FOR _ship IN
    SELECT s.*
  ...
  LOOP
    ...
    -- HP gate: stop badly damaged ships, never collect for them
    _hp_ratio := 1;
    ...
    -- Strict: only finalize when timer actually reached zero
    IF _elapsed < _duration THEN CONTINUE; END IF;

    -- If market is full we cannot store fish — leave ship as-is (finished but uncollected)
    IF _market_full THEN CONTINUE; END IF;

    -- Same crew bonuses a manual collect would use; GF does not invent extras
    SELECT EXISTS (
    ...
    IF _pool_len <= 0 THEN
      -- nothing to catch; just restart to keep ship turning
      UPDATE public.ships_owned
    ...
    IF _qty < 1 THEN
      -- market just filled mid-loop; stop processing further ships
      _market_full := true;
      CONTINUE;
    END IF;
    ...
    -- Immediately send ship back out (Golden Fisher's whole job)
    UPDATE public.ships_owned
       SET at_sea = true,
