...
DO $$
DECLARE
  v_user uuid := 'ae23acda-af29-4fb5-96fd-67e5003ece73';
  v_code text := 'ZZTEST' || floor(random()*100000)::text;
  v_id uuid;
  t0 timestamptz;
  v_ms numeric;
  v_res jsonb;
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO public._redeem_test_log(ms, ok, msg) VALUES (v_ms, true, left(v_res::text, 300));
  EXCEPTION WHEN OTHERS THEN
    v_ms := extract(epoch from (clock_timestamp() - t0)) * 1000;
    INSERT INTO public._redeem_test_log(ms, ok, msg) VALUES (v_ms, false, SQLERRM);
  END;

  PERFORM set_config('request.jwt.claims', '', true);

  -- cleanup: restore inventory exactly, remove test code traces
