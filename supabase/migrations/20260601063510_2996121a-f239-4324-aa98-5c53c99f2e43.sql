...
BEGIN
  ...
  ELSE
    ...
    ELSIF _is_trader THEN
      -- Activate / extend recipient's market trader for 10h
      ...
      ON CONFLICT (user_id) DO UPDATE
        SET trader_until = GREATEST(
            ...
            updated_at = now();
    ELSE
      -- Non-fixer/non-trader crew: prevent duplicate of same crew type on this ship
    ...
    VALUES (_recipient_id,
      CASE WHEN _is_fixer THEN '🛠️ مصلّح وصلك!'
           WHEN _is_trader THEN '💰 تاجر سوق وصلك!'
           ELSE '👨‍✈️ طاقم دعم وصل!' END,
      _sender_emoji || ' ' || _sender_name || ' أرسل لك ' || _crew_id ||
      CASE WHEN _is_trader THEN ' — تاجر السوق فعّال 10 ساعات' ELSE ' على سفينتك' END,
      'support', _me);
  END IF;
END;
$function$;