...
-- Update open_lucky_box to support new 'fatak' tier (50% stronger than divine)
  ...
  ELSIF v_prize.prize_type = 'dragon_equipment' THEN
    ...
    v_eq_name := CASE v_prize.item_type
      WHEN 'weapon' THEN CASE v_prize.item_id
        ...
        WHEN 'fatak'  THEN 'النصل الفتّاك 🩸' END
      ...
      ELSE CASE v_prize.item_id
        ...
        WHEN 'fatak'  THEN 'تميمة الفتك 🩸' END
    END;
...
-- Seed fatak prizes (one per slot) in the legendary bucket. Inactive by default — admins enable them.
INSERT INTO public.lucky_box_prizes (rarity, prize_type, item_type, item_id, amount, label, icon, weight, active) VALUES
('legendary','dragon_equipment', 'weapon',   'fatak', 1, 'فتاك — سلاح 🩸',  '⚔️', 1, false),
('legendary','dragon_equipment', 'armor',    'fatak', 1, 'فتاك — درع 🩸',   '🛡️', 1, false),
('legendary','dragon_equipment', 'talisman', 'fatak', 1, 'فتاك — تميمة 🩸', '📿', 1, false);
