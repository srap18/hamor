...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- FIX: real per-instance capacity for upgradeable submarines lives in max_hp
      ...
      END);
      -- Apply HP scaling (same as normal fishing)
      _capacity := GREATEST(1, FLOOR(_capacity * _hp_ratio)::bigint);

      _qty := _capacity * _luck_mult::bigint * _cycles::bigint;
