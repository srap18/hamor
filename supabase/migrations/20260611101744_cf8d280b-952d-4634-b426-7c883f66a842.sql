...
BEGIN
  ...
  -- CRITICAL: if storage is full, do NOT stop the ship. Roll back the whole
  -- transaction by raising — fishing_started_at remains intact so the ship
  -- keeps fishing and the user can sell, then collect.
  IF _market_remaining <= 0 THEN
    RAISE EXCEPTION 'market_full';
  END IF;

  -- Server-side time with +5s network-latency grace period and ROUND (not FLOOR)
  -- so a player who taps "collect" at 99% gets the full reward instead of losing
  -- a fish per percent of clock skew.
