...
BEGIN
  ...
  IF _market_remaining <= 0 THEN
    -- Market full: stop fishing entirely (dock ships, clear timers). No cycles accumulate.
  ...
  IF _market_full THEN
    -- Market filled during this tick: dock every ship so nothing keeps counting.
    UPDATE public.ships_owned SET at_sea = false, fishing_started_at = NULL, last_fishing_reward_at = NULL
     WHERE user_id = _user AND COALESCE(in_storage, false) = false AND stealing_target_user_id IS NULL AND stealing_ends_at IS NULL
       AND (COALESCE(at_sea,false) OR fishing_started_at IS NOT NULL OR last_fishing_reward_at IS NOT NULL);
  END IF;
