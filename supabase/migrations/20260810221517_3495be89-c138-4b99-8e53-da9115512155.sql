...
BEGIN
  FOR r IN
    ...
    WHERE n.nspname='public' AND p.prokind='f'
      AND pg_get_functiondef(p.oid) LIKE '%elite_vip_[1-5]_monthly%'
  LOOP
    EXECUTE replace(r.def, 'elite_vip_[1-5]_monthly', 'elite_vip_[1-6]_monthly');
  END LOOP;
END $mig$;
...
DECLARE _uid uuid := auth.uid(); _offer uuid; _cnt int; r record;
        ...
        _give_kinds int := 0; _want_kinds int := 0; _fee int := 50;
BEGIN
  ...
  IF public.elite_vip6_active(_uid) THEN _fee := 0; END IF;
  IF _fee > 0 THEN
    PERFORM public._mutate_currency(_uid, 0, -_fee, 0, 0);
  END IF;
  ...
  VALUES (_offer, _uid, 'created',
          jsonb_build_object('give', _give, 'want', _want, 'hours', LEAST(_hours,12), 'fee_gems', _fee),
          public._client_ip(), public._client_ua());
