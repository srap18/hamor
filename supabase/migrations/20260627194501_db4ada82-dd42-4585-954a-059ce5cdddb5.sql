...
BEGIN
  ...
  -- IMPORTANT: only rewind ships that are NOT already fishing.
  -- Ships already at sea keep their existing fishing_started_at /
  -- last_fishing_reward_at, otherwise re-activation would grant a free
  -- instant catch cycle and fill the ship immediately.
  ...
  -- Make sure any ship already mid-fishing stays marked at_sea (no rewind).
  UPDATE public.ships_owned
     SET at_sea = true
   WHERE user_id = _uid
