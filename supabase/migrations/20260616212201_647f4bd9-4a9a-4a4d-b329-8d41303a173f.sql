...
BEGIN
  ...
  IF _current IS NOT NULL AND _current > now() THEN
    -- ensure shield matches remaining golden fisher window
    UPDATE public.profiles
       SET protection_until = GREATEST(COALESCE(protection_until, _current), _current)
     WHERE id = _uid;
...
-- Backfill shield for anyone currently active
UPDATE public.profiles
