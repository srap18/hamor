...
-- Fix weekly XP: use fixed amounts per weapon, not damage amount.
-- Damage event still works because it reads attacks.damage_dealt independently.

UPDATE public.weapons_catalog SET xp = 200 WHERE id = 'ad_bomb';
UPDATE public.weapons_catalog SET xp = 50  WHERE id = 'rocket_large';
UPDATE public.weapons_catalog SET xp = 250 WHERE id = 'nuke';

-- regular weapon attack: XP = weapons_catalog.xp (fixed), only when damage > 0
...
-- nuke: fixed XP from weapons_catalog (250)
...
BEGIN
  ...
  IF _ships_hit > 0 AND _weapon_xp > 0 THEN
    UPDATE public.profiles SET xp = COALESCE(xp,0) + _weapon_xp WHERE id = _attacker;
  END IF;
...
-- ad_bomb: fixed XP from weapons_catalog (200)
