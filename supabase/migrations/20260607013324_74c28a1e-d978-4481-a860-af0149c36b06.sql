-- 1) Fix gem exploit: triggers must run as INVOKER so current_user = authenticated
ALTER FUNCTION public._protect_profile_currency() SECURITY INVOKER;
ALTER FUNCTION public.guard_profiles_self_update() SECURITY INVOKER;
ALTER FUNCTION public.guard_profiles_update() SECURITY INVOKER;

-- 2) Harden launch_ad_bomb: same attacker/target checks as apply_ship_damage
...
BEGIN
  ...
  -- Attacker must be eligible to attack (same rules as regular weapons)
