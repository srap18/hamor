...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS steal_blocked_until timestamptz;
...
RETURNS TABLE(blocked_until timestamptz)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
...
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'ship not found'; END IF;
  IF _ship.stealing_target_user_id IS DISTINCT FROM _me THEN
    RAISE EXCEPTION 'not your target';
  END IF;

  _until := now() + interval '1 hour';

  UPDATE public.profiles SET steal_blocked_until = _until WHERE id = _ship.user_id;
