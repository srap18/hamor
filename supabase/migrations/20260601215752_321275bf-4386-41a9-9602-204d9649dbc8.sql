...
BEGIN
  ...
  -- If not finished and not forced, refuse.
  ...
  -- If forced early stop, compute proportional ratio of completion
  IF _force AND _my_ship.stealing_ends_at IS NOT NULL AND _my_ship.stealing_ends_at > now() THEN
    -- Use stealing_target_ship_id+attacker_ship_id heuristic: total duration approximated from fishing_seconds*2
  ...
  _max := GREATEST(1, FLOOR(_max * _ratio)::int);
  IF NOT _force THEN _max := GREATEST(1, _max); END IF;

  -- Pick fish from target's pool first
  WITH pool_ids AS (
