...
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS golden_fisher_last_activated_at timestamptz;
...
BEGIN
  ...
  SELECT golden_fisher_until, golden_fisher_last_activated_at
    INTO _current, _last
  FROM public.profiles
  WHERE id = _uid
  FOR UPDATE;

  -- If still active, return without consuming.
  ...
  -- Daily cap: cannot reactivate within 24h of the last activation start.
  IF _last IS NOT NULL AND _last > (now() - interval '24 hours') THEN
    RAISE EXCEPTION 'daily_limit: golden fisher already used today (next available at %)', (_last + interval '24 hours');
  END IF;
