...
BEGIN
  ...
  -- Same-device guard: block if any account tied to any of this user's known devices already redeemed this code.
  IF EXISTS (
    SELECT 1
  ...
  IF v_row.extra_rewards IS NOT NULL AND jsonb_typeof(v_row.extra_rewards) = 'array' THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(v_row.extra_rewards) LOOP
      ...
      ELSIF v_t = 'dragon_equipment' THEN
        ...
        END LOOP;
      ELSIF v_t = 'market_level' THEN
        v_target_lvl := LEAST(31, GREATEST(1, COALESCE((v_item->>'level')::int, 1)));
        INSERT INTO public.user_market (user_id, level)
        VALUES (v_user, v_target_lvl)
        ON CONFLICT (user_id) DO UPDATE
          SET level = GREATEST(public.user_market.level, EXCLUDED.level),
              upgrading_to = NULL, upgrade_started_at = NULL,
              upgrade_ends_at = NULL, upgrade_cost_coins = 0,
              updated_at = now();
      ELSIF v_t = 'fish_market_level' THEN
        v_target_lvl := GREATEST(1, COALESCE((v_item->>'level')::int, 1));
        INSERT INTO public.user_fish_market (user_id, level)
        VALUES (v_user, v_target_lvl)
        ON CONFLICT (user_id) DO UPDATE
          SET level = GREATEST(public.user_fish_market.level, EXCLUDED.level),
              upgrading_to = NULL, upgrade_started_at = NULL,
              upgrade_ends_at = NULL, upgrade_cost_coins = 0,
              updated_at = now();
      END IF;
