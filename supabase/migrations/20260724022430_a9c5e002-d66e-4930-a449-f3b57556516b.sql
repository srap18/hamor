...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.device_slots ds_me
     ...
     WHERE ds_me.user_id = v_user AND ds_other.user_id <> v_user AND cr.code_id = v_row.id
  ) THEN RAISE EXCEPTION 'already_redeemed_on_this_device'; END IF;

  IF EXISTS (
    SELECT 1 FROM public.device_history dh_me
     ...
     WHERE dh_me.user_id = v_user AND dh_other.user_id <> v_user AND cr.code_id = v_row.id
       AND length(dh_me.device_id) >= 32 AND length(dh_other.device_id) >= 32
  ) THEN RAISE EXCEPTION 'already_redeemed_on_this_device'; END IF;

  IF EXISTS (
    SELECT 1 FROM public.device_accounts da_me
     ...
     WHERE da_me.user_id = v_user AND da_other.user_id <> v_user AND cr.code_id = v_row.id
  ) THEN RAISE EXCEPTION 'already_redeemed_on_this_device'; END IF;
  ...
  ELSIF v_row.reward_type = 'lootbox' AND v_row.item_id IS NOT NULL THEN
    FOR i IN 1..v_qty LOOP
      INSERT INTO public.lootbox_owned (user_id, type_id, opened) VALUES (v_user, v_row.item_id, false);
    END LOOP;
  ...
  ELSIF v_row.reward_type = 'vip' AND v_row.item_id IS NOT NULL THEN
    ...
    ELSE v_new_expires := GREATEST(COALESCE(v_cur_expires, now()), now()) + (v_qty || ' days')::interval;
    END IF;
    UPDATE public.profiles
  ...
  ELSIF v_row.reward_type = 'elite_vip' AND v_row.item_id IS NOT NULL THEN
    ...
    ELSE v_new_elite_expires := GREATEST(COALESCE(v_cur_elite_expires, now()), now()) + (v_qty || ' days')::interval;
    END IF;
    UPDATE public.profiles
    ...
    IF v_row.meta ? 'items' AND jsonb_typeof(v_row.meta->'items') = 'array' THEN
      FOR v_item IN SELECT * FROM jsonb_array_elements(v_row.meta->'items') LOOP
        ...
        IF v_t = 'currency' THEN
          UPDATE public.profiles SET coins = coins + v_icoins, gems = gems + v_igems, xp = xp + v_ixp WHERE id = v_user;
        ...
        ELSIF v_t = 'ship' AND v_iid IS NOT NULL THEN
          ...
          FOR i IN 1..v_iqty LOOP PERFORM public.buy_ship_by_code(v_iid, v_template_id, 0, 100000); END LOOP;
        ELSIF v_t = 'lootbox' AND v_iid IS NOT NULL THEN
          FOR i IN 1..v_iqty LOOP INSERT INTO public.lootbox_owned (user_id, type_id, opened) VALUES (v_user, v_iid, false); END LOOP;
  ...
  -- NEW: reward_vip_level / reward_vip_days
  ...
  -- NEW: reward_elite_vip_level / reward_elite_vip_days
