...
BEGIN
  ...
  IF COALESCE(_lvl,1) < 6 THEN
    RETURN jsonb_build_object('status','level_locked','current_level',COALESCE(_lvl,1),'required_level',6,
      'message','لا تقدر ترسل إلا بعد وصول سوق السفن للمستوى 6');
  END IF;
