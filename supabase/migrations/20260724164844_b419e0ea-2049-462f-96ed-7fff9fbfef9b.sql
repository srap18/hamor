...
BEGIN
  ...
  -- Null-out no-action FKs pointing back to this user from admin/audit-style tables,
  -- otherwise the final profile/auth delete fails on FK violation.
  UPDATE public.admin_audit    SET target_user_id = NULL WHERE target_user_id = _uid;
  UPDATE public.bans           SET banned_by      = NULL WHERE banned_by      = _uid;
  UPDATE public.device_appeals SET resolved_by    = NULL WHERE resolved_by    = _uid;
  UPDATE public.ludo_rooms     SET winner_id      = NULL WHERE winner_id      = _uid;
