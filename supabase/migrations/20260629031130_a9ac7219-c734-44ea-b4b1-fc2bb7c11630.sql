...
BEGIN
  ...
  IF NOT FOUND OR NOT v_settings.enabled THEN RAISE EXCEPTION 'box_disabled'; END IF;
  v_cost := v_settings.cost_gems;
  ...
  v_roll := floor(random() * GREATEST(1, v_settings.pct_common + v_settings.pct_rare + v_settings.pct_legendary))::int;
  ...
  IF v_prize.prize_type = 'coins' THEN
    UPDATE public.profiles SET coins = coins + v_prize.amount WHERE id = v_user;
  ELSIF v_prize.prize_type = 'gems' THEN
    UPDATE public.profiles SET gems = gems + v_prize.amount WHERE id = v_user;
  ELSIF v_prize.prize_type = 'rubies' THEN
    UPDATE public.profiles SET rubies = rubies + v_prize.amount WHERE id = v_user;
  ...
  ELSIF v_prize.prize_type = 'item' THEN
    ...
    ON CONFLICT (user_id, item_type, item_id) DO UPDATE SET quantity = public.inventory.quantity + EXCLUDED.quantity;
  ...
  -- Personal notification — only for rare/legendary (common stays silent)
  ...
  v_result := jsonb_build_object('ok', true, 'rarity', v_rarity, 'prize_id', v_prize.id,
    ...
    'amount', v_prize.amount, 'gems_balance', v_balance);
  RETURN v_result;
END;
$$;