-- 1) Anti speed-hack & anti early-collect: require full server-side duration,
--    and remove the "1 fish minimum" floor.
...
BEGIN
  ...
  -- ANTI-CHEAT: no "minimum 1 fish" floor anymore. Early stop or speed-hack = 0 fish.
...
-- 2) Anti dual-device timer reset: do NOT reset fishing_started_at if the ship
--    is already at sea. Setting at_sea=true twice keeps the original start time,
--    so an attacker on a second device can't push the timer back to "ready".
...
BEGIN
  ...
  -- Destroyed/repairing ships can't be sent fishing.
  ...
  IF _at_sea THEN
    -- If already at sea with an active trip, keep the original start time.
