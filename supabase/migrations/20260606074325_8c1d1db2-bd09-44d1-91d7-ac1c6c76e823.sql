...
BEGIN
  ...
  -- XP from fishing disabled — XP only via attacks/bombing
  _xp := 0;
...
-- Reset XP and level for all players (bypass guard trigger via session flag if needed)
DO $$
BEGIN
  PERFORM set_config('app.bypass_profile_guard', '1', true);
  UPDATE public.profiles SET xp = 0, level = 1, weekly_xp = 0;
