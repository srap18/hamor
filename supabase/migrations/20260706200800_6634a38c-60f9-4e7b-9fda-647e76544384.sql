...
DROP FUNCTION IF EXISTS public.ludo_quick_match();
DROP FUNCTION IF EXISTS public.ludo_quick_match(int);

CREATE OR REPLACE FUNCTION public.ludo_quick_match(_players int DEFAULT 2)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.ludo_quick_match(int) TO authenticated;
