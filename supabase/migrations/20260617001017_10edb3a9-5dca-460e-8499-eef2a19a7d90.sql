...
BEGIN
  ...
  -- 100% guide accuracy priority chain:
  -- 1. Live request from client (when guide is currently assigned)
  -- 2. Persisted ship.preferred_fish_id (even if guide row missing — covers crew expiry/race)
  -- 3. Guide inventory meta preferred
  -- 4. Random fallback
  ...
  -- Persist chosen for future ticks so golden fisher stays aligned
