-- Force old fish-market clients to update and move selling to a versioned RPC.
...
BEGIN
  ...
  PERFORM set_config('app.audit_reason', 'fish_sale_versioned', true);
...
GRANT EXECUTE ON FUNCTION public.sell_fish_by_qty(text, integer, text) TO authenticated;
REVOKE EXECUTE ON FUNCTION public.sell_fish_by_qty(text, integer, text) FROM anon, public;