...
BEGIN
  ...
  SELECT protection_until INTO _prot FROM public.profiles WHERE id = _ship.stealing_target_user_id;
  IF _prot IS NOT NULL AND _prot > now() THEN
    UPDATE public.ships_owned
       SET at_sea = false, fishing_started_at = NULL,
           stealing_target_user_id = NULL, stealing_target_ship_id = NULL, stealing_ends_at = NULL
  ...
  IF _scaled > 0 THEN
    ...
    WHERE so.id = _ship.stealing_target_ship_id
      AND so.user_id = _ship.stealing_target_user_id;
    ...
    picked AS (
      SELECT fs.id, fs.base_value
       ...
       WHERE fs.user_id = _ship.stealing_target_user_id
