...
-- Block stacking Golden Fisher: cannot activate or receive as gift while already active.
...
BEGIN
  ...
  -- HARD BLOCK: if already active, refuse — do not consume inventory, do not stack.
...
-- Patch send_support: block gifting golden_fisher to a recipient who already has it active.
...
BEGIN
  ...
  -- HARD BLOCK: recipient already has active Golden Fisher — don't consume sender's item.
  ...
  ELSE
    ...
    ELSIF _is_golden_fisher THEN
      -- Fresh 24h; no shield (shield removed globally).
