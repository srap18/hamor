...
ALTER TABLE public.ludo_rooms
  ADD COLUMN IF NOT EXISTS consecutive_sixes int NOT NULL DEFAULT 0;
...
BEGIN
  ...
  -- Rule: three 6s in a row -> forfeit the turn (dice ignored, no move).
...
-- Reset the sixes counter whenever the turn passes to another seat via move_token.
