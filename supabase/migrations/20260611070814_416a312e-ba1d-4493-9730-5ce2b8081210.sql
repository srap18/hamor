...
DECLARE
  ...
  _has_sailor boolean := false;
  _sailor_assigned_at timestamptz;
  _bonus_seconds numeric := 0;
...
BEGIN
  ...
  -- Sailor: STRICTLY scoped to this ship (assigned_ship_id = THIS ship's UUID),
  -- and not expired. Read assigned_at to decide if the boost applies to the
  -- whole trip or only to the post-assignment portion (no retroactive boost).
  SELECT
    true,
    NULLIF(inv.meta->>'assigned_at','')::timestamptz
  ...
  -- Luck & Guide (also strictly scoped to this ship)
  SELECT EXISTS (
    SELECT 1 FROM public.inventory inv
    WHERE inv.user_id = _uid AND inv.item_type='crew' AND inv.item_id='luck'
      AND inv.meta->>'assigned_ship_id' = _ship_id::text
      AND ((inv.meta->>'expires_at') IS NULL OR (inv.meta->>'expires_at')::timestamptz > now())
  ) INTO _has_crew;
  ...
  -- Effective elapsed: base elapsed + sailor bonus.
  -- Sailor doubles the rate, applied:
  --   * over the whole trip if assigned BEFORE (or at) trip start (or legacy/no assigned_at)
  --   * only on the time AFTER assignment if assigned mid-trip
  _elapsed := EXTRACT(EPOCH FROM (now() - _ship.fishing_started_at));
  IF _has_sailor THEN
    IF _sailor_assigned_at IS NULL OR _sailor_assigned_at <= _ship.fishing_started_at THEN
      _bonus_seconds := _elapsed; -- full 2x
    ELSE
      _bonus_seconds := GREATEST(0, EXTRACT(EPOCH FROM (now() - _sailor_assigned_at)));
    END IF;
    _elapsed := _elapsed + _bonus_seconds;
  END IF;
  _elapsed := GREATEST(0, _elapsed);
