...
-- Fix golden fisher shield in record_attack (both overloads)
...
AS $function$
DECLARE _uid uuid := auth.uid(); _id uuid; _def_prot timestamptz; _def_gf timestamptz; _att_gf timestamptz; _mult numeric;
BEGIN
  ...
  IF (_def_prot IS NOT NULL AND _def_prot > now()) OR (_def_gf IS NOT NULL AND _def_gf > now()) THEN
    -- ensure protection_until reflects active golden fisher
  ...
  -- Clear attacker's protection, but never below their active golden_fisher_until
  SELECT golden_fisher_until INTO _att_gf FROM public.profiles WHERE id = _uid;
  IF _att_gf IS NOT NULL AND _att_gf > now() THEN
    UPDATE public.profiles SET protection_until = _att_gf
      WHERE id = _uid AND (protection_until IS NULL OR protection_until > _att_gf);
  ELSE
...
CREATE OR REPLACE FUNCTION public.record_attack(_defender_id uuid, _target_ship_id uuid, _damage integer, _damage_dealt integer, _attacker_won boolean, _xp_gain integer DEFAULT 0)
 ...
 SET search_path TO 'public'
AS $function$
DECLARE _uid uuid := auth.uid(); _id uuid; _xp int; _def_prot timestamptz; _def_gf timestamptz; _att_gf timestamptz; _mult numeric;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'not authenticated'; END IF;
  IF _defender_id IS NULL OR _defender_id = _uid THEN RAISE EXCEPTION 'invalid defender'; END IF;
  IF _damage < 0 OR _damage > 10000000 THEN RAISE EXCEPTION 'bad damage'; END IF;
...
-- Sync protection_until for all currently-active golden fisher holders
UPDATE public.profiles
   SET protection_until = GREATEST(COALESCE(protection_until, golden_fisher_until), golden_fisher_until)
 WHERE golden_fisher_until IS NOT NULL AND golden_fisher_until > now()
   AND (protection_until IS NULL OR protection_until < golden_fisher_until);
