...
BEGIN
  ...
  LOOP
    ...
    -- Determine elapsed time since the last completed cycle to grant
    -- multiple cycles in one tick (golden fisher should keep filling
    -- the market up to capacity, not just one batch per ship).
    _last_at := COALESCE(_ship.last_fishing_reward_at, _ship.fishing_started_at, _now - (_duration || ' seconds')::interval);
    _elapsed := EXTRACT(EPOCH FROM (_now - _last_at));
    _cycles := GREATEST(1, FLOOR(_elapsed / _duration)::int);
    -- Safety cap so a long-idle ship doesn't dump millions in one call
    ...
    _qty := GREATEST(1, COALESCE(_ship.storage, _ship.fishing_power, 1))::bigint
            * _luck_mult::bigint
            * _cycles::bigint;

    -- Cap by remaining market capacity
    IF _qty > _market_remaining THEN
      _qty := _market_remaining;
      _market_full := true;
    END IF;
    IF _qty < 1 THEN
      CONTINUE;
    ...
    VALUES (_user, _chosen, _ship.id, _now, _unit_value, _qty::int);
