...
CREATE POLICY "atk_select_involved" ON public.attacks FOR SELECT
  USING (auth.uid() = attacker_id OR auth.uid() = defender_id);
CREATE POLICY "atk_insert_attacker" ON public.attacks FOR INSERT
  WITH CHECK (auth.uid() = attacker_id AND attacker_id <> defender_id);
