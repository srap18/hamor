-- Auto-equip newly redeemed frames when the matching profile slot is empty,
-- so users immediately see the frame after redeeming a code.
...
BEGIN
  ...
  ELSIF v_row.reward_type = 'item' AND v_row.item_id IS NOT NULL THEN
    ...
    -- Auto-equip cosmetic frames if the slot is empty
    IF COALESCE(v_row.item_kind,'') = 'frame' THEN
      UPDATE public.profiles SET avatar_frame = v_row.item_id
        WHERE id = v_user AND (avatar_frame IS NULL OR avatar_frame = '');
    ELSIF COALESCE(v_row.item_kind,'') = 'name_frame' THEN
      UPDATE public.profiles SET name_frame = v_row.item_id
        WHERE id = v_user AND (name_frame IS NULL OR name_frame = '');
    ELSIF COALESCE(v_row.item_kind,'') = 'bubble_frame' THEN
      UPDATE public.profiles SET bubble_frame = v_row.item_id
        WHERE id = v_user AND (bubble_frame IS NULL OR bubble_frame = '');
    ELSIF COALESCE(v_row.item_kind,'') = 'profile_frame' THEN
      UPDATE public.profiles SET profile_frame = v_row.item_id
        WHERE id = v_user AND (profile_frame IS NULL OR profile_frame = '');
    END IF;
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(v_row.extra_rewards) LOOP
      ...
      ELSIF v_t = 'item' THEN
        ...
        IF v_iid IS NOT NULL THEN
          ...
          -- Auto-equip cosmetic frames if the slot is empty
          IF v_ikind = 'frame' THEN
            UPDATE public.profiles SET avatar_frame = v_iid
              WHERE id = v_user AND (avatar_frame IS NULL OR avatar_frame = '');
          ELSIF v_ikind = 'name_frame' THEN
            UPDATE public.profiles SET name_frame = v_iid
              WHERE id = v_user AND (name_frame IS NULL OR name_frame = '');
          ELSIF v_ikind = 'bubble_frame' THEN
            UPDATE public.profiles SET bubble_frame = v_iid
              WHERE id = v_user AND (bubble_frame IS NULL OR bubble_frame = '');
          ELSIF v_ikind = 'profile_frame' THEN
            UPDATE public.profiles SET profile_frame = v_iid
              WHERE id = v_user AND (profile_frame IS NULL OR profile_frame = '');
          END IF;
...
-- Backfill: auto-equip af_leo (and other cosmetic frames) for users who
-- redeemed the frame codes but never equipped anything.
UPDATE public.profiles p SET avatar_frame = i.item_id
  FROM public.inventory i
 WHERE i.user_id = p.id
   AND i.item_type = 'frame'
   AND (p.avatar_frame IS NULL OR p.avatar_frame = '');

UPDATE public.profiles p SET name_frame = i.item_id
  FROM public.inventory i
 WHERE i.user_id = p.id
   AND i.item_type = 'name_frame'
   AND (p.name_frame IS NULL OR p.name_frame = '');

UPDATE public.profiles p SET bubble_frame = i.item_id
  FROM public.inventory i
 WHERE i.user_id = p.id
   AND i.item_type = 'bubble_frame'
   AND (p.bubble_frame IS NULL OR p.bubble_frame = '');

UPDATE public.profiles p SET profile_frame = i.item_id
  FROM public.inventory i
 WHERE i.user_id = p.id
   AND i.item_type = 'profile_frame'
   AND (p.profile_frame IS NULL OR p.profile_frame = '');