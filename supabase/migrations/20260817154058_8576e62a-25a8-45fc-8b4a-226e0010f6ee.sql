CREATE OR REPLACE FUNCTION public.elite_vip_active(_uid uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.profiles p
    WHERE p.id = _uid
      AND COALESCE(p.elite_vip_level, 0) >= 1
      AND (p.elite_vip_expires_at IS NULL OR p.elite_vip_expires_at > now())
  );
$$;
GRANT EXECUTE ON FUNCTION public.elite_vip_active(uuid) TO authenticated, service_role;
...
BEGIN
  ...
  IF public.elite_vip_active(v_user) THEN
    v_purchase_limit := 100;
  END IF;
