-- 1) Record assigned_at when crew is sent as support (prevents retroactive sailor bonus)
...
DECLARE
  _me uuid := auth.uid();
  _sender_name text; _sender_emoji text;
  _ship_owner uuid;
  _inv_id uuid; _crew_qty int;
  _msg text; _title text; _body_action text;
  _expires timestamptz := now() + interval '24 hours';
  _is_fixer boolean; _is_fixer_legendary boolean;
  _is_trader boolean; _is_market_expert boolean;
  _is_golden_fisher boolean; _is_persistent boolean;
  _heal int := 0; _affected int := 0;
  _trader_ends timestamptz; _crew_label text;
  _gf_current timestamptz; _gf_new_until timestamptz;
  _req_error text;
BEGIN
  ...
  IF NOT public.is_admin(_me) THEN
    _req_error := public.pvp_support_pair_error(_me, _recipient_id);
  ...
  IF _kind = 'repair' THEN
    UPDATE public.ships_owned
       SET hp = max_hp, destroyed_at = NULL, repair_ends_at = NULL,
           passive_repair_ends_at = NULL, repair_started_hp = NULL
  ...
  IF _is_persistent THEN
    ...
    VALUES (_recipient_id, 'crew', _crew_id, 1,
            jsonb_build_object('assigned_ship_id', _ship_id::text, 'assigned_at', now(), 'expires_at', _expires));
    ...
    ELSIF _is_fixer THEN
      UPDATE public.ships_owned
         SET hp = LEAST(max_hp, COALESCE(hp,0) + _heal),
             ...
             passive_repair_ends_at = CASE WHEN COALESCE(hp,0) + _heal >= max_hp THEN NULL ELSE passive_repair_ends_at END,
             repair_started_hp = CASE WHEN COALESCE(hp,0) + _heal >= max_hp THEN NULL
                                      ELSE LEAST(max_hp, COALESCE(hp,0) + _heal) END
       WHERE id = _ship_id AND user_id = _recipient_id;
      _msg := 'مصلح: +' || _heal || ' HP';
...
-- 2) Backfill assigned_at for assigned crew rows missing it (derive from expires_at - 24h)
UPDATE public.inventory
   SET meta = meta || jsonb_build_object(
         'assigned_at',
         to_jsonb(GREATEST(
           (NULLIF(meta->>'expires_at','')::timestamptz - interval '24 hours'),
           now() - interval '24 hours'
         ))
       )
 WHERE item_type = 'crew'
   AND meta->>'assigned_ship_id' IS NOT NULL
   AND (meta->>'assigned_at') IS NULL
   AND NULLIF(meta->>'expires_at','') IS NOT NULL;

UPDATE public.inventory
   SET meta = meta || jsonb_build_object('assigned_at', to_jsonb(now()))
 WHERE item_type = 'crew'
   AND meta->>'assigned_ship_id' IS NOT NULL
   AND (meta->>'assigned_at') IS NULL;

-- 3) Clean expired crew rows still attached to ships
DELETE FROM public.inventory
 WHERE item_type = 'crew'
   AND meta->>'assigned_ship_id' IS NOT NULL
   AND NULLIF(meta->>'expires_at','') IS NOT NULL
...
-- 4) Clear stale passive repair timers on ships that are already at full HP
UPDATE public.ships_owned
   SET passive_repair_ends_at = NULL, repair_started_hp = NULL, repair_ends_at = NULL
 WHERE destroyed_at IS NULL
   AND COALESCE(hp,0) >= COALESCE(max_hp,0)
   AND (passive_repair_ends_at IS NOT NULL OR repair_started_hp IS NOT NULL OR repair_ends_at IS NOT NULL);

-- 5) Lock down internal helpers
REVOKE ALL ON FUNCTION public._crew_return_to_pool(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._crew_return_to_pool(uuid) TO service_role;
