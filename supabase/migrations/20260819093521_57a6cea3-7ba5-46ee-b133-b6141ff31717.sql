...
BEGIN
  ...
  IF COALESCE(_s.catalog_code,'') IN ('submarine','upgrade-sub','royal-whale')
     OR COALESCE(_s.template_id,0) IN (32,33,37) THEN
    ...
    WHILE _retry AND _attempt < 3 LOOP
      ...
      BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM public.ship_catalog c WHERE c.active AND c.code = _resolved_code) THEN
          _resolved_code := CASE COALESCE(_ship_locked.template_id, 0) WHEN 31 THEN 'phoenix' WHEN 32 THEN 'submarine' WHEN 33 THEN 'upgrade-sub' WHEN 37 THEN 'royal-whale' ELSE _resolved_code END;
        END IF;
        ...
        _capacity := GREATEST(1, CASE
          WHEN COALESCE(_ship_locked.catalog_code,'') IN ('submarine','upgrade-sub','royal-whale')
            OR COALESCE(_ship_locked.template_id,0) IN (32,33,37)
          THEN COALESCE(_ship_locked.max_hp, _capacity, _qty, 1)::bigint
          ELSE COALESCE(_capacity, _qty, 1)::bigint END);
