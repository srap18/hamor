...
BEGIN
  ...
  -- NEW: If market is already full at the start of the tick, halt all
  -- fishing for this user (dock every ship). Player must empty the market
  -- to resume — the next tick after emptying will re-launch the ships.
  ...
  -- NEW: When market fills mid-tick, halt fishing entirely — dock every
  -- ship so the player sees ships waiting at port and the on-screen
  -- warning "empty the market" is unambiguous.
