-- Update start_steal_mission: scale duration by attacker ship fishing_power
-- and stamp fishing_started_at = now() so early-cancel yields proportional fish.
...
BEGIN
  ...
  -- Duration scales with attacker ship's fishing power (its "points").
  -- Higher power → longer mission window → more fish capacity.
  ...
  UPDATE public.ships_owned
     SET at_sea = true,
         -- Stamp the mission start so early-recall yields proportional fish.
...
-- Update cancel_steal_mission: yield is pure proportional (no minimum-1 fish floor),
-- mirroring fishing exactly. Fish type still comes from target ship's fish_pool.
...
BEGIN
  ...
  IF _ship.user_id = _me THEN
    -- Pure proportional: no minimum floor. Stop at 0s = 0 fish, like fishing.
