...
BEGIN
  ...
  -- Base = time-based fill toward ship capacity.
  -- If the player sailed for at least 2 seconds and has storage left,
  -- guarantee at least 1 fish so early-stopping always returns something.
  _base := FLOOR(_capacity * _ratio)::integer;
  IF _base <= 0 AND _elapsed >= 2 AND _remaining_cap > 0 THEN
    _base := 1;
  END IF;
  _base := GREATEST(_base, 0);
  _base := LEAST(_base, _remaining_cap);
