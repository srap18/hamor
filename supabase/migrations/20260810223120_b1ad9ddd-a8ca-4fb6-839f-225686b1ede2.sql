...
BEGIN
  ...
  v_elite_level := CASE
    WHEN _pack_id ~ '^elite_vip_[1-6]_monthly$'
      THEN substring(_pack_id from 'elite_vip_([1-6])_monthly')::int
    ELSE NULL
  END;
...
CREATE OR REPLACE FUNCTION public.resync_my_elite_vip()
 RETURNS TABLE(level integer, expires_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  IF _pack IS NOT NULL THEN
    _pack_level := substring(_pack from 'elite_vip_([1-6])_monthly')::int;
