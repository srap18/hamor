...
BEGIN
  ...
  LOOP
    ...
    -- Priority: guide's preferred fish (player's chosen) > ship's saved preference > deterministic.
    -- Never override the guide's selection.
    ...
    -- Sync ship.preferred_fish_id with guide's selection, but never overwrite guide's saved choice.
    ...
    IF _qty < _capacity THEN
      _market_full := true;
      _ships_waiting_for_space := _ships_waiting_for_space + 1;
      UPDATE public.ships_owned SET at_sea = true WHERE id = _ship.id AND at_sea = false;
      _ships_processed := _ships_processed + 1;
      CONTINUE;
    END IF;
