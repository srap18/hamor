...
BEGIN
  ...
  IF _scaled > 0 THEN
    ...
    LOOP
      ...
      VALUES (_me, _row.fish_id, _take, _take, now())
...
BEGIN
  ...
  IF _scaled > 0 THEN
    ...
    LOOP
      ...
      INSERT INTO public.fish_caught(user_id, fish_id, quantity, total_caught, updated_at)
      VALUES (_ship.user_id, _row.fish_id, _take, _take, now())
      ON CONFLICT (user_id, fish_id) DO UPDATE
      SET quantity = public.fish_caught.quantity + EXCLUDED.quantity,
          total_caught = public.fish_caught.total_caught + EXCLUDED.total_caught,
          updated_at = now();
...
BEGIN
  ...
  IF COALESCE(_row.stolen_count, 0) <> 1 OR _after_attacker <> _before_attacker + 1 OR _after_defender <> _before_defender - 1 THEN
    RAISE EXCEPTION 'claim steal one fish test failed';
  END IF;
...
BEGIN
  ...
  IF COALESCE(_row.stolen_count, 0) <> 1 OR _after_attacker <> _before_attacker + 1 OR _after_defender <> _before_defender - 1 THEN
    RAISE EXCEPTION 'cancel steal one fish test failed';
  END IF;
