...
BEGIN
  ...
  -- Guide crew: also read the stored preferred fish so picks survive across
  -- devices / cleared localStorage / stale client state.
  ...
  -- 100% accuracy when guide is active: prefer the explicit request, then the
  -- stored preference. Only fall back to RNG if no guide at all.
  ...
  -- If the guide produced a pick, persist it back as the preferred fish so
  -- the Golden Fisher auto-tick and future collects stay in sync.
  IF _has_guide AND _chosen IS NOT NULL AND _chosen IS DISTINCT FROM _guide_pref THEN
  ...
  IF _market_remaining <= 0 THEN _qty := 0; END IF;
  _qty := LEAST(_qty, GREATEST(0, _market_remaining)::integer);

  IF _qty > 0 THEN
    INSERT INTO public.fish_caught (user_id, fish_id, quantity, total_caught, updated_at)
    VALUES (_uid, _chosen, _qty, _qty, now())
  ...
  _xp := GREATEST(0, _qty);
  IF _xp > 0 THEN
    PERFORM public.add_xp(_uid, _xp);
  END IF;
