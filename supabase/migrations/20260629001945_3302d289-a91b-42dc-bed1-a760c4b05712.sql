-- Allow trusted game RPCs to perform protected prize/currency writes during their transaction only
...
-- Dragon equipment can now store the fatak rarity granted by lucky boxes
ALTER TABLE public.dragon_equipment
  DROP CONSTRAINT IF EXISTS dragon_equipment_rarity_check;

ALTER TABLE public.dragon_equipment
  ADD CONSTRAINT dragon_equipment_rarity_check
  CHECK (rarity IN ('common','rare','epic','legendary','divine','fatak'));

-- Fix lucky box opening: use the correct lucky_box_rarity enum, authorize protected writes,
-- support fatak grants, and return fresh balance/count data to the app.
...
BEGIN
  ...
  -- This flag is transaction-local and lets existing protection triggers know
  -- this write is coming from the official server-side game RPC.
  ...
  SET gems = COALESCE(gems, 0) - v_settings.cost_gems
  WHERE id = v_user;

  v_roll := random() * GREATEST(1, v_settings.pct_common + v_settings.pct_rare + v_settings.pct_legendary);
  IF v_roll < v_settings.pct_legendary THEN
    v_rarity := 'legendary';
  ELSIF v_roll < v_settings.pct_legendary + v_settings.pct_rare THEN
    v_rarity := 'rare';
  ELSE
    v_rarity := 'common';
  END IF;
  ...
  ELSIF v_prize.prize_type = 'dragon_equipment' THEN
    ...
    v_eq_name := CASE v_prize.item_type
      WHEN 'weapon' THEN CASE v_prize.item_id
        ...
        WHEN 'divine' THEN 'النصل الأبدي 🔱'
        WHEN 'fatak' THEN 'النصل الفتّاك 🩸'
      END
      WHEN 'armor' THEN CASE v_prize.item_id
        ...
        WHEN 'divine' THEN 'درع الأبدية 🔱'
        WHEN 'fatak' THEN 'درع الفتك 🩸'
      END
      ELSE CASE v_prize.item_id
        ...
        WHEN 'divine' THEN 'تميمة الأبدية 🔱'
        WHEN 'fatak' THEN 'تميمة الفتك 🩸'
      END
    END;
  ...
  SELECT COALESCE(gems, 0) INTO v_gems_left
  FROM public.profiles
  WHERE id = v_user;

  SELECT COUNT(*)::int INTO v_opens_count
  FROM public.lucky_box_opens
  WHERE user_id = v_user;
  ...
  v_result := jsonb_build_object(
    ...
    'gems_left', v_gems_left,
    'opens_count', v_opens_count
  );
