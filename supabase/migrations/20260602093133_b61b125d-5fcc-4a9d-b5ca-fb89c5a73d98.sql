-- Cap per-trip quantity inside collect_fishing_reward so large-storage ships return instantly
...
DECLARE
  ...
  _stock_cap constant integer := 5000; -- safety cap on fish_stock rows per trip
BEGIN
  ...
  -- Hard cap to keep fish_stock insert fast and ensure results always appear
  IF _qty > _stock_cap THEN _qty := _stock_cap; END IF;
  IF _base > _stock_cap THEN _base := _stock_cap; END IF;
  ...
  SELECT _uid, _chosen, _ship_id, now(), _unit_value FROM generate_series(1, _qty);
  ...
  fish_id := _chosen;
  fish_qty := _inserted_count;
  base_qty := _base;
  luck_bonus := GREATEST(0, _inserted_count - _base);
  xp_awarded := _xp;
