...
BEGIN
  ...
  END IF;
  -- Raised from 800ms to reduce spurious lock_timeout errors during heavy contention.
  ...
  LOOP
    ...
    WHILE _retry AND _attempt < 3 LOOP
      ...
      BEGIN
        ...
        -- SKIP LOCKED: if the ship row is being touched by another
        -- transaction (repair, sale, fishing action), skip it this tick
        -- and try again on the next tick instead of surfacing a lock_timeout error.
      ...
      EXCEPTION
        WHEN deadlock_detected OR lock_not_available THEN
          -- Contention only — try again on the next tick, do not log as error.
