...
-- Compensation: +1 golden_fisher for every user who activated it in the last 48 hours.
DO $$
DECLARE _u uuid; BEGIN
  FOR _u IN SELECT id FROM public.profiles WHERE golden_fisher_last_activated_at >= now() - interval '48 hours' LOOP
    UPDATE public.inventory
       SET quantity = quantity + 1
     WHERE user_id = _u AND item_type='crew' AND item_id='golden_fisher'
    ...
    IF NOT FOUND THEN
      ...
      VALUES (_u, 'crew', 'golden_fisher', 1, '{}'::jsonb, now());
    END IF;
  END LOOP;
END $$;
