...
-- Threshold helper: ship is "fishable" once repair progress >= 30%
CREATE OR REPLACE FUNCTION public._ship_repair_ratio(_destroyed_at timestamptz, _repair_ends_at timestamptz)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
...
-- set_ship_at_sea: allow fishing when repair progress >= 30%
...
-- collect_fishing_reward: scale capacity by repair ratio, keep repair timer intact
...
BEGIN
  ...
  -- Scale capacity by current repair progress (effective HP)
  ...
  -- Stop the fishing trip but DO NOT cancel the ongoing repair
