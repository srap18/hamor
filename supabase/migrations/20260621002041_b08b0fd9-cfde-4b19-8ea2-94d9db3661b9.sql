...
BEGIN
  ...
  -- Always award the current partial loot to the attacker ship owner, even if
  -- the defender is the one who stopped the raid. Previously defender-stops
  -- forced _scaled to 0, making every interrupted raid return empty.
  SELECT COALESCE(SUM(GREATEST(0, quantity)), 0)::int INTO _existing
  FROM public.fish_stock WHERE user_id = _attacker_user_id AND ship_id = _attacker_ship_id;
  ...
  IF _scaled > 0 THEN
    ...
    LOOP
      ...
      VALUES (_attacker_user_id, _row.fish_id, _attacker_ship_id, now(), _row.base_value, _take);
      INSERT INTO public.fish_caught(user_id, fish_id, quantity, total_caught, updated_at)
      VALUES (_attacker_user_id, _row.fish_id, _take, _take, now())
      ...
      VALUES (_attacker_user_id, _row.fish_id, now(), _take);
    ...
    IF _remaining > 0 AND _target_ship.id IS NOT NULL AND _target_ship.fishing_started_at IS NOT NULL THEN
      ...
      IF _pool_len > 0 THEN
        ...
        IF _take > 0 THEN
          ...
          FROM public.fish_market_prices WHERE fish_market_prices.fish_id = _generated_fish_id;
          INSERT INTO public.fish_stock(user_id, fish_id, ship_id, caught_at, base_value, quantity)
          VALUES (_attacker_user_id, _generated_fish_id, _attacker_ship_id, now(), COALESCE(_unit_value, 0), _take);
          INSERT INTO public.fish_caught(user_id, fish_id, quantity, total_caught, updated_at)
          VALUES (_attacker_user_id, _generated_fish_id, _take, _take, now())
          ON CONFLICT (user_id, fish_id) DO UPDATE
          SET quantity = public.fish_caught.quantity + EXCLUDED.quantity,
              total_caught = public.fish_caught.total_caught + EXCLUDED.total_caught,
              updated_at = now();
          INSERT INTO public.competition_catches(user_id, fish_id, caught_at, qty)
          VALUES (_attacker_user_id, _generated_fish_id, now(), _take);
