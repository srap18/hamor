...
CREATE OR REPLACE FUNCTION public.admin_recent_chat_senders(_limit int DEFAULT 10, _since timestamptz DEFAULT NULL)
RETURNS TABLE (
  sender_id uuid,
  display_name text,
  avatar_url text,
...
GRANT EXECUTE ON FUNCTION public.admin_recent_chat_senders(int, timestamptz) TO authenticated;
