...
-- Enforce only ONE active trader and ONE active guide globally per user.
-- Also keep the existing per-ship uniqueness for other crew types.
...
BEGIN
  ...
  -- purge expired same-type same-ship rows
  ...
  -- Global lock for trader/guide: only one active across all ships
  IF _crew_id IN ('trader','guide') THEN
    IF EXISTS (
    ...
    END IF;
  ELSE
    -- Per-ship uniqueness for other crews
    IF EXISTS (
      SELECT 1 FROM public.inventory
      WHERE user_id = _uid
        AND item_type = 'crew'
