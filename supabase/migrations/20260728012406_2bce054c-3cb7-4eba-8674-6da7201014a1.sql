...
-- Max ship level counts EVERY owned ship (storage, destroyed, docked included)
...
-- Gap protection: based on max owned levels; level-20 shipyard exception; bidirectional; opens on engagement
...
BEGIN
  ...
  -- Engagement window (bidirectional): if either side hit the other in the last
  -- 30 minutes, PvP is open regardless of level gap. This makes retaliation immediate.
  SELECT EXISTS (
    SELECT 1 FROM public.attacks a
    WHERE (
        (a.attacker_id = _attacker AND a.defender_id = _defender)
     OR (a.attacker_id = _defender AND a.defender_id = _attacker)
    )
    AND a.created_at > now() - interval '30 minutes'
  ) INTO _recent;
  IF _recent THEN RETURN NULL; END IF;
  ...
  -- Level-20 shipyard exception: the LOWER side, once at market level 20+,
  -- loses gap protection against the higher side (and can hit up in return).
...
-- Support: only sender is restricted; threshold raised to 11.
CREATE OR REPLACE FUNCTION public.pvp_support_requirement_error(_user_id uuid, _actor_label text DEFAULT 'sender')
RETURNS text
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = 'public'
AS $$
DECLARE _market int;
BEGIN
  ...
  IF _market < 11 THEN
    RETURN COALESCE(_actor_label,'sender') || ' يجب أن يكون مستوى سوق السفن ١١ أو أعلى: الحالي=' || _market::text;
  END IF;
  RETURN NULL;
END $$;

-- Rewrite send_support_impl: remove recipient level check; keep everything else.
...
BEGIN
  ...
  IF NOT public.is_admin(_me) THEN
    -- Sender-only requirement (market level >= 11). No restriction on recipient.
