...
-- Golden Fisher: auto-resume fishing for eligible ships every tick.
-- After a ship is destroyed and fully repaired, finalize_ship_repairs() resets
-- at_sea=false and fishing_started_at=null, so the next golden_fisher_tick
-- found no ships to process and effectively "disabled" itself until the
-- player manually re-launched the ships. We now ensure each tick re-arms all
-- eligible ships into fishing (same logic as activate_golden_fisher).
...
BEGIN
  ...
  -- Auto-resume fishing for every eligible ship of this user that is not
  -- already fishing. Mirrors the logic in activate_golden_fisher so that
  -- ships continue to fish automatically after a destroy → repair cycle.
