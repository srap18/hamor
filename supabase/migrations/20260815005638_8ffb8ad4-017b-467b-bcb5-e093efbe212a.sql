  ...
  ELSIF k = 'tribe_event' THEN
    ...
    IF v_tribe IS NOT NULL THEN
      SELECT COUNT(*) + 1 INTO v_rank FROM (
        SELECT m.id AS user_id, (CASE
          WHEN ev.metric = 'gold' THEN COALESCE((SELECT SUM(g.amount) FROM public.tribe_fish_event_gold g WHERE g.event_id = _ref AND g.user_id = m.id),0)
          WHEN ev.metric IN ('damage','destroy') THEN COALESCE((
              SELECT SUM(CASE WHEN ev.metric='destroy' THEN (CASE WHEN a.attacker_won THEN 1 ELSE 0 END)::bigint ELSE GREATEST(a.damage_dealt,0)::bigint END)
              FROM public.attacks a
              LEFT JOIN public.profiles pd2 ON pd2.id = a.defender_id
             WHERE a.attacker_id = m.id
               AND COALESCE(a.attacker_tribe_id, v_tribe) IS NOT DISTINCT FROM v_tribe
               AND a.attacker_id <> a.defender_id
               AND COALESCE(a.defender_tribe_id, pd2.tribe_id) IS DISTINCT FROM v_tribe
               AND a.created_at >= ev.starts_at AND a.created_at <= ev.ends_at),0)
          ELSE COALESCE((SELECT SUM(cc.qty) FROM public.competition_catches cc WHERE cc.user_id = m.id AND cc.caught_at >= ev.starts_at AND cc.caught_at <= ev.ends_at),0)
        END)::bigint AS sc
