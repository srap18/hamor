...
BEGIN
  ...
  -- Golden Fisher owns fishing while active: clear stealing routes touching this user.
  ...
  LOOP
    ...
    BEGIN
      ...
      SELECT s.*, c.fish_pool, c.fishing_seconds, c.storage, c.fishing_power
        INTO _ship_locked
        FROM public.ships_owned s
        LEFT JOIN public.ship_catalog c
          ON c.active = true
         AND c.code = COALESCE(s.catalog_code, 'ship-lvl-' || COALESCE(s.template_id, 1)::text)
       WHERE s.id = _ship.id
       FOR UPDATE OF s;
      ...
      -- If the ship is not already on a trip, launch it only. Do not grant fish
      -- immediately and do not keep an old last_fishing_reward_at from a prior trip.
      ...
      -- Critical fix: do not force a cycle before it is complete. The old
      -- GREATEST(1, ...) path reserved a reward slot, reset the timer, and then
      -- future refreshes found the slot already used so no fish reached storage.
      IF _cycles <= 0 THEN
        UPDATE public.ships_owned
           SET at_sea = true
         WHERE id = _ship_locked.id;
      ...
      -- Use epoch-based completion keys instead of duration buckets. This avoids
      -- collisions when boosted ships complete faster than their base duration.
      ...
      IF _inserted_slots <= 0 THEN
        -- Already rewarded by a concurrent/previous tick; advance the marker so
        -- the same completed cycle is not retried forever.
        UPDATE public.ships_owned
           SET at_sea = true,
               fishing_started_at = _new_last_at,
      ...
      _capacity := GREATEST(1, CASE
        WHEN COALESCE(_ship_locked.catalog_code,'') IN ('submarine','upgrade-sub')
          ...
          THEN COALESCE(_ship_locked.max_hp, _ship_locked.storage, _ship_locked.fishing_power, 1)::bigint
        ELSE COALESCE(_ship_locked.storage, _ship_locked.fishing_power, 1)::bigint
      END);
      ...
      IF _last_inserted_slot IS NOT NULL THEN
        UPDATE public.golden_fisher_rewards
         ...
         WHERE ship_id = _ship_locked.id
           AND reward_slot = _last_inserted_slot;
      END IF;
