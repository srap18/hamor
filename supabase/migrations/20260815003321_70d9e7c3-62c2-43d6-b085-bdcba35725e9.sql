CREATE OR REPLACE FUNCTION public.my_leaderboard_rank(_kind text, _ref uuid DEFAULT NULL)
...
BEGIN
  ...
  IF k = 'competition' THEN
    ...
    WITH board AS (
      SELECT x.uid AS pid, x.sc AS sc FROM (
        SELECT a.attacker_id AS uid, COUNT(*)::bigint AS sc
      ...
      WHERE NOT public.is_admin(x.uid) AND x.sc > 0
    )
  ...
  ELSIF k = 'tribe_event' THEN
    ...
    IF ev.metric = 'gold' THEN
      SELECT COALESCE(SUM(g.amount),0)::bigint INTO v_score
        FROM public.tribe_fish_event_gold g WHERE g.event_id = _ref AND g.user_id = uid;
    ELSIF ev.metric IN ('damage','destroy') THEN
      SELECT COALESCE(SUM(CASE WHEN ev.metric = 'destroy'
                    ...
                    ELSE GREATEST(a.damage_dealt,0)::bigint END),0)::bigint INTO v_score
        FROM public.attacks a
       WHERE a.attacker_id = uid AND a.attacker_tribe_id = v_tribe
    ...
    ELSE
      SELECT COALESCE(SUM(cc.qty),0)::bigint INTO v_score
        FROM public.competition_catches cc
       WHERE cc.user_id = uid AND cc.caught_at >= ev.starts_at AND cc.caught_at <= ev.ends_at;
    END IF;
    v_score := COALESCE(v_score,0);
    IF v_tribe IS NOT NULL THEN
      SELECT COUNT(*) + 1 INTO v_rank FROM (
        SELECT m.user_id, (CASE
          WHEN ev.metric = 'gold' THEN COALESCE((SELECT SUM(g.amount) FROM public.tribe_fish_event_gold g WHERE g.event_id = _ref AND g.user_id = m.user_id),0)
          WHEN ev.metric IN ('damage','destroy') THEN COALESCE((SELECT SUM(CASE WHEN ev.metric='destroy' THEN (CASE WHEN a.attacker_won THEN 1 ELSE 0 END)::bigint ELSE GREATEST(a.damage_dealt,0)::bigint END)
              FROM public.attacks a WHERE a.attacker_id = m.user_id AND a.attacker_tribe_id = v_tribe
                AND a.attacker_id <> a.defender_id AND (a.defender_tribe_id IS DISTINCT FROM a.attacker_tribe_id)
                AND a.created_at >= ev.starts_at AND a.created_at <= ev.ends_at),0)
          ELSE COALESCE((SELECT SUM(cc.qty) FROM public.competition_catches cc WHERE cc.user_id = m.user_id AND cc.caught_at >= ev.starts_at AND cc.caught_at <= ev.ends_at),0)
        END)::bigint AS sc
...
REVOKE ALL ON FUNCTION public.my_leaderboard_rank(text, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.my_leaderboard_rank(text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_leaderboard_rank(text, uuid) TO service_role;