...
-- 1) Data fix: phoenix ships with wrong catalog_code
UPDATE public.ships_owned
 ...
 WHERE template_id = 31
   AND COALESCE(catalog_code,'') <> 'phoenix';

-- 2) Defensive fix in golden_fisher_tick:
--    add a fallback so a missing catalog row never silently consumes cycles.
...
BEGIN
  ...
  IF _market_remaining <= 0 THEN
    ...
    RETURN jsonb_build_object(
      'ok', true, 'ships_processed', 0, 'launched', 0,
      'cycles', 0, 'fish_added', 0,
      'waiting_for_space', 0, 'market_full', true
    );
  END IF;
  ...
  LOOP
    ...
    WHILE _retry AND _attempt < 3 LOOP
      ...
      BEGIN
        ...
        -- Resolve catalog with smart fallback: explicit code → ship-lvl-N →
        -- phoenix(31) → submarine(32/33). Self-heal the row if needed so the
        -- next tick uses the corrected code directly.
        _resolved_code := COALESCE(NULLIF(_ship_locked.catalog_code,''),
                                   'ship-lvl-' || COALESCE(_ship_locked.template_id,1)::text);
        ...
        IF NOT EXISTS (SELECT 1 FROM public.ship_catalog c WHERE c.active AND c.code = _resolved_code) THEN
          _resolved_code := CASE COALESCE(_ship_locked.template_id, 0)
            WHEN 31 THEN 'phoenix'
            WHEN 32 THEN 'submarine'
            WHEN 33 THEN 'upgrade-sub'
            ELSE _resolved_code
          END;
        ...
        SELECT c.fish_pool, c.fishing_seconds, c.storage, c.fishing_power
         ...
         WHERE c.active = true AND c.code = _resolved_code
         LIMIT 1;

        -- If we still have no catalog match, log and skip this ship instead
        -- of silently relaunching it (the previous behavior consumed cycles
        -- without ever producing fish).
        IF _pool IS NULL THEN
          INSERT INTO public.golden_fisher_errors(
            ...
            _user, _ship_locked.id, 0, 0, _market_remaining,
        ...
        IF NOT COALESCE(_ship_locked.at_sea, false) OR _ship_locked.fishing_started_at IS NULL THEN
          UPDATE public.ships_owned
             SET at_sea = true, fishing_started_at = _now, last_fishing_reward_at = NULL
           WHERE id = _ship_locked.id;
        ...
        _last_at := GREATEST(
          COALESCE(_ship_locked.last_fishing_reward_at, _ship_locked.fishing_started_at),
        ...
        IF _inserted_slots <= 0 THEN
          UPDATE public.ships_owned
             SET at_sea = true, fishing_started_at = _new_last_at, last_fishing_reward_at = _new_last_at
           WHERE id = _ship_locked.id;
          EXIT;
        END IF;
        ...
        IF _pool_len <= 0 THEN
          INSERT INTO public.golden_fisher_errors(
            ...
            _user, _ship_locked.id, _cycles, 0, _market_remaining,
            EXTRACT(MILLISECONDS FROM (clock_timestamp() - _t_start))::int,
        ...
        ELSE
          SELECT p.value INTO _chosen
           ...
           WHERE p.ord = (1 + (abs(hashtextextended(_ship_locked.id::text || ':' || _last_at::text, 71003)) % _pool_len))
           LIMIT 1;
        END IF;
        ...
        _capacity := GREATEST(1, CASE
          WHEN COALESCE(_ship_locked.catalog_code,'') IN ('submarine','upgrade-sub')
            OR COALESCE(_ship_locked.template_id,0) IN (32,33)
        ...
        _qty := _capacity * _luck_mult::bigint * _inserted_slots::bigint;
      ...
      EXCEPTION
        WHEN deadlock_detected OR lock_not_available THEN
          IF _attempt < 3 THEN
            _retry := true;
            PERFORM pg_sleep(0.05 * _attempt);
          ELSE
            INSERT INTO public.golden_fisher_errors(
              user_id, ship_id, cycles, fish_added, remaining_storage, exec_ms, error
            ) VALUES (
  ...
  RETURN jsonb_build_object(
    'ok', true, 'ships_processed', _ships_processed, 'launched', _ships_launched,
    'cycles', _total_cycles, 'fish_added', _total_fish,
    'waiting_for_space', 0, 'market_full', _market_full
  );
END;
$function$;
