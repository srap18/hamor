...
CREATE OR REPLACE FUNCTION public.revoke_paddle_purchase(
  ...
  _block_account boolean DEFAULT false
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  _body := 'تم استرداد مبلغ عملية شراء سابقة، وبموجب سياسة الاسترداد تم سحب جميع المكافآت الممنوحة (قد يصبح رصيدك بالسالب حتى يُعوَّض).';
  IF _block_account THEN
    _body := _body || ' كما تم تعليق إمكانية الشراء على حسابك.';
  END IF;
