-- 1) Remove the legacy quota-bypassing boss attack RPC
REVOKE ALL ON FUNCTION public.attack_boss(boolean) FROM anon, authenticated;
DROP FUNCTION IF EXISTS public.attack_boss(boolean);

-- 2) Unify boss-destruction repair time with the normal repair formula
...
BEGIN
  ...
  IF v_destroyed THEN
    -- same repair duration as any other destruction (3h .. 24h)
    v_repair_secs := public._owned_ship_repair_seconds(v_ship.template_id, v_ship.catalog_code);
