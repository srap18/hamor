...
BEGIN
  FOR r IN SELECT fish_id, max_ship_level, rarity_rank FROM public.fish_ship_max_level LOOP
    ...
    -- IDEMPOTENCY GUARD: only advance once per hour. If the row was already
    -- advanced this hour, skip it so duplicate cron firings or manual calls
    -- do not push the forecast forward an extra step.
    IF prev_last IS NOT NULL AND date_trunc('hour', prev_last) >= current_hour THEN
      CONTINUE;
    END IF;

    -- 1) Materialize forecast[0] verbatim as the new current price.
    ...
    -- 3) Shift forecast forward by exactly one and append one fresh step.
    new_fc := '[]'::jsonb;
