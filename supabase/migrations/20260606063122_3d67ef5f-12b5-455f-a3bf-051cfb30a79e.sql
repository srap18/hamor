...
BEGIN
  ...
  VALUES (
    _target_user_id,
    '🏴‍☠️ محاولة سرقة!',
    _attacker_emoji || ' ' || _attacker_name || ' بدأ يسرق من سفينتك',
    'attack',
    _me,
    jsonb_build_object('ship_id', _target_ship_id, 'attacker_ship_id', _attacker_ship_id, 'ends_at', _ends, 'event', 'steal_started')
  );
