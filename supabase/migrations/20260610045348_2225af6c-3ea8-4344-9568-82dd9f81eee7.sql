...
-- 1) New columns on redemption_codes
ALTER TABLE public.redemption_codes
  ADD COLUMN IF NOT EXISTS reward_elite_vip_level integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS reward_elite_vip_days  integer NOT NULL DEFAULT 0;

ALTER TABLE public.redemption_codes
  DROP CONSTRAINT IF EXISTS redemption_codes_elite_vip_level_check;
ALTER TABLE public.redemption_codes
  ADD CONSTRAINT redemption_codes_elite_vip_level_check
    CHECK (reward_elite_vip_level >= 0 AND reward_elite_vip_level <= 5);

-- 2) Elite VIP expiry column on profiles (NULL = permanent, future ts = active)
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS elite_vip_expires_at timestamptz;

-- 3) Update redeem_code to also grant Elite VIP when configured on the code
...
BEGIN
  ...
  -- Elite VIP (5-tier subscription system) — granted via admin code only
  IF v_row.reward_elite_vip_level IS NOT NULL AND v_row.reward_elite_vip_level > 0 THEN
    ...
    -- Always upgrade to the HIGHEST tier (do not stack levels for Elite)
