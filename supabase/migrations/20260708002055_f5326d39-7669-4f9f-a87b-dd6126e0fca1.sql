...
BEGIN
  ...
  -- Remove waiting rooms if they are old or nobody in them is currently online.
  DELETE FROM public.ludo_rooms r
  ...
  -- Remove playing rooms if every player has been offline for a short period.
  DELETE FROM public.ludo_rooms r
...
-- Keep the newer helper name as an alias so any caller runs the same cleanup logic.
