...
BEGIN
  ...
  -- Pre-age eligible ships so every tick yields a full cycle instantly while
  -- golden fisher is active. Only touch ships whose timer is missing or hasn't
  -- reached one full duration yet — never push backward an already-ready ship.
  UPDATE public.ships_owned s
   ...
   WHERE c.code = s.catalog_code
     ...
     AND (
       s.at_sea = false
       OR s.fishing_started_at IS NULL
       OR s.fishing_started_at > (_now - (GREATEST(60, COALESCE(c.fishing_seconds, 600)) || ' seconds')::interval)
     );
  ...
  LOOP
    ...
    -- Only advance the timer when fish were actually caught. When the market
    -- is full and 0 cycles ran, leave fishing_started_at alone so the ship
    -- surfaces as "✅ ready" in the UI and the player can sell + collect.
    IF _consumed_cycles > 0 THEN
      UPDATE public.ships_owned
         SET fishing_started_at = (_now - (_duration || ' seconds')::interval)
       WHERE id = _ship.id;
  ...
  RETURN jsonb_build_object(
    ...
    'market_full', _market_was_full AND _total_cycles = 0
  );
END;
$function$;