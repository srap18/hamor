...
BEGIN
  ...
  -- IMPORTANT: only tick ships the player currently has at sea AND that already
  -- started a trip. This respects manual "stop" actions — without it the tick
  -- would re-launch a just-docked ship every few seconds, causing the UI to
  -- bounce / appear to hang when the user collects then re-sends a ship.
  FOR _ship IN
  ...
  LOOP
    ...
    ) THEN _sailor_mult := 2; END IF;

    _elapsed := GREATEST(0, EXTRACT(EPOCH FROM (_now - _ship.fishing_started_at))::int);
    _threshold := (_duration::numeric / _sailor_mult) * 0.99;
