...
-- Close double-sell exploit: sell_fish_caught and sell_fish (by ids) can credit coins
-- without removing the corresponding fish_stock rows, allowing players to sell the
-- same inventory twice. Revoke from authenticated and drop the orphan helper.

REVOKE EXECUTE ON FUNCTION public.sell_fish_caught(text, integer, numeric) FROM authenticated, anon, public;
REVOKE EXECUTE ON FUNCTION public.sell_fish(uuid[]) FROM authenticated, anon, public;
REVOKE EXECUTE ON FUNCTION public.increment_fish_caught(text, integer) FROM authenticated, anon, public;

-- Strengthen sell_fish_by_qty: full per-user serialization (not just per-fish),
-- shorter window so double-tap can't get two RPCs through, and re-quote happens
...
BEGIN
  ...
  -- Per-user lock so concurrent sells (even across fish types) are fully serialized.
  ...
  -- Recount from rows actually mutated, not from the planning CTE.
