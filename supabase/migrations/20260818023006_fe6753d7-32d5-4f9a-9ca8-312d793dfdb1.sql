CREATE INDEX IF NOT EXISTS idx_chat_mutes_user_active_expiry
  ON public.chat_mutes (user_id, created_at DESC)
  WHERE active = true;

CREATE INDEX IF NOT EXISTS idx_chat_mute_devices_active_expiry
  ON public.chat_mute_devices (device_id, created_at DESC)
  WHERE active = true;

CREATE INDEX IF NOT EXISTS idx_chat_mute_ips_active_expiry
  ON public.chat_mute_ips (ip, created_at DESC)
  WHERE active = true;
...
BEGIN
  ...
  BEGIN
    PERFORM set_config('lock_timeout', '40ms', true);
