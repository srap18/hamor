...
BEGIN
  ...
  -- Pre-age ALL eligible ships so every tick yields a full cycle instantly
  -- (golden fisher behavior: instant catch, even for ships mid-cycle).
  ...
  LOOP
    ...
    -- Reset timer so the NEXT tick is also instant while golden fisher is active.
    -- If market was full this run, still pre-age so the user isn't stuck waiting.
    UPDATE public.ships_owned
