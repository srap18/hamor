...
BEGIN
  ...
  SELECT protection_until, shield_cooldown_until INTO v_prot, v_cd
  ...
  -- Short global anti-spam cooldown (e.g. after dropping a shield) still applies.
  IF NOT v_no_wait AND v_cd IS NOT NULL AND v_cd > now()
     AND v_cd <= now() + interval '10 minutes' THEN
  ...
  -- SECURITY FIX: do NOT wipe the paid global cooldown for free; only clear
  -- the short anti-spam window that has already elapsed.
  UPDATE public.profiles
     SET protection_until = v_new,
         shield_cooldown_until = CASE
           WHEN shield_cooldown_until IS NULL THEN NULL
           WHEN shield_cooldown_until <= now() THEN NULL
           ELSE shield_cooldown_until END
   WHERE id = v_user;
...
BEGIN
  ...
  SELECT COALESCE(gems, 0), shield_cooldown_until
    INTO v_gems, v_cd
    FROM public.profiles WHERE id = v_user FOR UPDATE;
  ...
  -- SECURITY FIX: the remaining wait is the LONGEST of both sources, so a
  -- never-activated shield type can no longer be used to clear a paid
  -- global cooldown for free.
  v_until := GREATEST(
    COALESCE(v_last + interval '7 days', to_timestamp(0)),
    COALESCE(v_cd, to_timestamp(0))
  );

  -- Nothing pending, or only the short anti-spam window (<5 min): free no-op.
  IF v_until <= now() + interval '5 minutes' THEN
    UPDATE public.profiles
       SET shield_cooldown_until = NULL
     WHERE id = v_user
       AND (shield_cooldown_until IS NULL OR shield_cooldown_until <= now() + interval '5 minutes');
    RETURN jsonb_build_object('ok', true, 'skipped', false, 'cost', 0);
  END IF;

  v_secs_left := EXTRACT(EPOCH FROM (v_until - now()))::int;
