...
BEGIN
  ...
  -- Global pity: every 200th box across ALL players = guaranteed legendary,
  -- every 50th box = guaranteed rare. Otherwise roll normally.
  ...
  IF v_rarity IN ('rare','legendary') THEN
    ...
    VALUES (
      ...
      jsonb_build_object('user_id', v_user, 'rarity', v_rarity, 'prize_name', v_prize.label, 'pity', v_pity, 'global_open', v_this_open)
    );
  END IF;
