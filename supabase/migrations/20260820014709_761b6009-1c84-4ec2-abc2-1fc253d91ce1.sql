...
CREATE OR REPLACE FUNCTION public.launch_doom_annihilator(_target_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  RETURN public.launch_doom_annihilator_impl(_target_id);
END $function$;

REVOKE ALL ON FUNCTION public.launch_doom_annihilator_impl(uuid) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.launch_doom_annihilator(uuid) TO authenticated;
