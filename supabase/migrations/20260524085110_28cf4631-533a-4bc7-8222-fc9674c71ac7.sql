...
-- everyone can read basic profiles (for friends search, leaderboard, chat names)
...
-- security-definer helper: is user in tribe?
...
create policy "friends_select_involved" on public.friends for select
  using (auth.uid() = requester_id or auth.uid() = addressee_id);
create policy "friends_insert_requester" on public.friends for insert
  with check (auth.uid() = requester_id and requester_id <> addressee_id);
create policy "friends_update_addressee" on public.friends for update
  using (auth.uid() = addressee_id or auth.uid() = requester_id);
create policy "friends_delete_involved" on public.friends for delete
  using (auth.uid() = requester_id or auth.uid() = addressee_id);
...
create policy "msg_select_public" on public.messages for select
  using (channel = 'public');
create policy "msg_select_dm" on public.messages for select
  using (channel = 'dm' and (auth.uid() = sender_id or auth.uid() = recipient_id));
create policy "msg_select_tribe" on public.messages for select
  using (channel = 'tribe' and tribe_id is not null and public.is_tribe_member(auth.uid(), tribe_id));

create policy "msg_insert_public" on public.messages for insert
  with check (channel = 'public' and auth.uid() = sender_id);
create policy "msg_insert_dm" on public.messages for insert
  with check (channel = 'dm' and auth.uid() = sender_id and recipient_id is not null);
create policy "msg_insert_tribe" on public.messages for insert
  with check (channel = 'tribe' and auth.uid() = sender_id and tribe_id is not null and public.is_tribe_member(auth.uid(), tribe_id));

-- ============= SHIP LISTINGS (secondary market) =============
...
create policy "listings_insert_seller" on public.ship_listings for insert
  with check (auth.uid() = seller_id);
create policy "listings_update_seller" on public.ship_listings for update
