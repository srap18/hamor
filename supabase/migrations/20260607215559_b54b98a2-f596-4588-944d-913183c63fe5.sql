...
-- Reduce ship repair duration: min 60s (L1) → max 14400s/4h (L30), linear by template level.
-- Also cap special ships (31 phoenix, 32 submarine) at 4h.
...
BEGIN
  ...
  SELECT protection_until INTO _prot FROM public.user_protection WHERE user_id = _owner;
  IF _prot IS NOT NULL AND _prot > now() THEN
    RAISE EXCEPTION 'target is under protection';
  END IF;

  _tpl := COALESCE(_tpl, 1);
  -- Linear ramp: L1 = 60s (1 min), L30 = 14400s (4 hours).
  -- Clamp template level to 1..30 for the formula so special ships (31, 32) cap at 4h.
...
-- Shorten any currently-pending repairs to the new max (4h from now).
