...
BEGIN
  ...
  LOOP
    ...
    -- Priority for chosen fish:
    -- 1. preferred_fish_id stored on the ship (set whenever the player picks
    --    a fish from the guide UI — this is the source of truth and what the
    --    foreground fishing flow uses).
    -- 2. preferred_fish_id mirrored into the guide inventory row (legacy).
    -- 3. Random from the ship's pool.
