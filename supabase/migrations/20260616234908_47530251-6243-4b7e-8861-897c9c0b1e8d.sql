...
-- 1) Enforce "one open ticket per user" at the DB level
CREATE UNIQUE INDEX IF NOT EXISTS support_tickets_one_open_per_user
  ON public.support_tickets (user_id)
  WHERE status IN ('open', 'in_progress');

-- 2) Relax launch_ad_bomb: drop the 3x level-6 PvP fleet requirement.
...
BEGIN
  ...
  -- NOTE: pvp fleet check removed — Ad Bomb is a premium one-shot item.
  ...
  INSERT INTO public.attacks (attacker_id, target_id, weapon_id, damage, ships_hit, kind, created_at)
  VALUES (_attacker, _target_id, 'ad_bomb', _total_damage, _ships_hit, 'ad_bomb', now())
