...
BEGIN
  ...
  IF NOT _freeze_active THEN _frozen_prices := '{}'::jsonb; END IF;
  ...
  priced AS (
    SELECT m.*,
      ...
      CASE
        ...
        -- live market price (same value the market UI quotes); fall back to
        -- catch-time value only when no live price exists
        ELSE GREATEST(1, COALESCE(NULLIF(m.fmp_cur, 0)::numeric, NULLIF(m.base_value, 0)::numeric, 1))
      END AS base_unit,
      CASE WHEN _frozen_prices ? m.fish_id
           THEN (_frozen_prices ->> m.fish_id)::numeric
           ELSE NULL END AS frozen_unit
  )
  SELECT
    COALESCE(SUM(
      GREATEST(
        ...
        LEAST(
          ...
          GREATEST(base_unit * rot, COALESCE(frozen_unit, 0))
        )
      )::bigint * quantity
    ), 0),
  ...
  IF _total > 0 THEN
    ...
    IF _total >= _audit_threshold THEN
      INSERT INTO public.transaction_logs(
        ...
        jsonb_build_object('sold_counts', _sold_counts, 'stock_ids_count', array_length(_fish_stock_ids,1), 'market_expert', _expert_active, 'rot_applied', true, 'freeze_active', _freeze_active)
      );
    END IF;
