...
BEGIN
  ...
  INSERT INTO public.economy_audit (
    ...
    _uid, -_cost, _gold, _gold - _cost, 'upgrade_submarine',
  ...
  _old_cap := GREATEST(1, COALESCE(_ship.max_hp, CASE _stars
    ...
    WHEN 4 THEN 850000 WHEN 5 THEN 1000000 ELSE 350000 END));

  -- Preserve the current health percentage. A fully healthy submarine remains
  -- fully healthy after upgrade, so its usable fishing capacity increases now.
  _new_hp := LEAST(
    _new_cap,
