...
DROP FUNCTION IF EXISTS public.broadcast_nuke(uuid, text);

-- Create updated function without inserting into notifications table
...
BEGIN
  ...
  -- Require a recent attack record (last 5 minutes)
  ...
  -- No notification insert; client broadcasts via realtime channel
  RETURN;
END;
$$;
