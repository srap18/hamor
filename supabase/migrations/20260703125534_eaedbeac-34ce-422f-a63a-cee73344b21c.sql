...
-- 1) Pause flag (does not affect golden_fisher_until timer)
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS golden_fisher_paused boolean NOT NULL DEFAULT false;

-- 2) Skip the tick while paused (early-return; timer keeps counting down)
...
-- 3) Pause: dock all ships so the player can freely swap. Timer keeps counting down.
...
BEGIN
  ...
  -- Dock all fishing ships so they can be swapped / stored freely.
  UPDATE public.ships_owned
     SET at_sea = false,
  ...
  RETURN jsonb_build_object('ok', true, 'paused', true, 'until', _until);
END;
$function$;

-- 4) Resume: clear pause and re-launch ships via one tick.
...
BEGIN
  ...
  UPDATE public.profiles SET golden_fisher_paused = false WHERE id = _uid;
...
GRANT EXECUTE ON FUNCTION public.pause_golden_fisher() TO authenticated;
GRANT EXECUTE ON FUNCTION public.resume_golden_fisher() TO authenticated;
