...
BEGIN
  ...
  IF _kind = 'repair' THEN
    ...
    -- Grant 60s attack immunity to recipient
  ...
  -- Grant 60s attack immunity to recipient when receiving any fixer crew
  IF _grant_immunity THEN
    UPDATE public.profiles
       SET protection_until = GREATEST(COALESCE(protection_until, now()), now() + interval '60 seconds')
     WHERE id = _recipient_id;
  END IF;
