...
BEGIN
  ...
  ELSE
    ...
    -- Trader: prevent duplicate active trader regardless of ship
    IF _crew_id = 'trader' THEN
      SELECT count(*) INTO _active_trader FROM public.inventory
        WHERE user_id = _recipient_id AND item_type = 'crew' AND item_id = 'trader'
          AND meta->>'assigned_ship_id' IS NOT NULL
          AND ((meta->>'expires_at') IS NULL OR (meta->>'expires_at')::timestamptz > now());
      IF _active_trader > 0 THEN
