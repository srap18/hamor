...
BEGIN
  ...
  SELECT wc.damage, COALESCE(wc.xp,0) INTO _base_damage, _weapon_xp
    FROM public.weapons_catalog wc WHERE wc.id = _weapon_id;
  ...
  IF _blocked THEN
    SELECT p.display_name INTO _attacker_name FROM public.profiles p WHERE p.id = _attacker;
    SELECT p.display_name INTO _defender_name FROM public.profiles p WHERE p.id = _defender;
