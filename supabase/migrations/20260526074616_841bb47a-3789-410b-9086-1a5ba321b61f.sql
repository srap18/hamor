...
BEGIN
  ...
  ELSE
    ...
    IF _is_fixer THEN
      -- Fixer: instantly repair ship
      UPDATE public.ships_owned
         SET hp = max_hp, destroyed_at = NULL, repair_ends_at = NULL
       WHERE id = _ship_id;
    ELSE
      -- Non-fixer crew: prevent duplicate of the same crew type on this ship
      ...
      -- Insert as ASSIGNED to the target ship directly (uses ship UUID).
      -- A unique-violation can occur if recipient happens to own an unassigned
      -- row of this crew with NULL meta (because of the UNIQUE (user_id,item_type,item_id)
      -- constraint when meta is NULL). In that case, claim that row by setting its meta.
      BEGIN
    ...
    _msg := 'طاقم دعم: ' || _crew_id;
    INSERT INTO public.support_gifts (sender_id, recipient_id, ship_id, kind, amount, message, claimed)
    VALUES (_me, _recipient_id, _ship_id, 'crew', 0, _msg, true);

    INSERT INTO public.notifications (recipient_id, title, body, kind, created_by)
    VALUES (_recipient_id,
      CASE WHEN _is_fixer THEN '🛠️ مصلّح وصلك!' ELSE '👨‍✈️ طاقم دعم وصل!' END,
      _sender_emoji || ' ' || _sender_name || ' أرسل لك ' || _crew_id || ' على سفينتك',
      'support', _me);
  END IF;
END;
$function$;