...
BEGIN
  ...
  IF _ship.preferred_fish_id IS NOT NULL AND _ship.preferred_fish_id <> ''
  ...
  ELSIF _owns_guide AND _requested_fish_id IS NOT NULL
...
-- Repair ships that already had an invalid preferred_fish_id (fish not in their pool)
UPDATE public.ships_owned so
   SET preferred_fish_id = NULL
  FROM public.ship_catalog c
 WHERE so.catalog_code = c.code
   AND so.preferred_fish_id IS NOT NULL
   AND so.preferred_fish_id <> ''
   AND NOT EXISTS (
     SELECT 1 FROM jsonb_array_elements_text(COALESCE(c.fish_pool, '[]'::jsonb)) v(fid)
      WHERE v.fid = so.preferred_fish_id
   );