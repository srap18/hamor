...
BEGIN
  ...
  -- Look up catalog row using the canonical pattern used elsewhere
  -- (catalog_code → ship-lvl-<template_id> → sort_order). ship_catalog has
  -- no `level` column; the old query was broken.
