...
/**
 * Floating, collapsible, draggable action button.
 ...
 */
export function DraggableActionButton({
  storageKey,
  onClick,
  label,
  collapsedIcon,
  ...
  title?: string;
  variant?: "emerald" | "fuchsia" | "rose" | "amber";
  ...
  const gradient =
    variant === "fuchsia"
      ? "from-fuchsia-400 to-fuchsia-700 border-fuchsia-200"
      : variant === "rose"
        ? "from-rose-400 to-rose-700 border-rose-200"
        : variant === "amber"
          ? "from-amber-400 to-amber-700 border-amber-200"
          : "from-emerald-400 to-emerald-700 border-emerald-200";
  ...
  }, [bounds, storageKey, defaultPosition?.x, defaultPosition?.y]);
          ...
          {open ? (
            <div className="flex items-center gap-1">
              <button
                ...
                className={`px-3 py-1.5 rounded-xl bg-gradient-to-b ${gradient} border-2 text-white text-xs font-extrabold shadow-2xl active:scale-95 flex items-center gap-1.5 animate-pulse`}
              >
                {label}
              </button>
              <button
                ...
                aria-label="طي"
                className="w-7 h-7 rounded-full bg-stone-900/90 border border-white/30 text-white text-xs font-black shadow-lg active:scale-95 flex items-center justify-center"
              >
                ×
              </button>
            </div>
            ...
            <button
              ...
              aria-label={typeof title === "string" ? title : "فتح"}
              className={`w-10 h-10 rounded-full bg-gradient-to-b ${gradient} border-2 text-white text-sm shadow-2xl active:scale-95 flex items-center justify-center animate-pulse`}
            >
              {collapsedIcon}
            </button>
          )}
