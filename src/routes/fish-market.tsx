...
function FishMarket() {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden text-white" dir="rtl">
      ...
      {/* TOP HUD */}
      <div className="absolute top-0 left-0 right-0 z-30 px-2 pb-2 pt-[max(0.5rem,var(--sa-top))] mt-10 flex flex-col gap-1.5">
          ...
          <div className="w-full max-w-sm rounded-2xl bg-gradient-to-b from-slate-800 to-slate-900 border-2 border-amber-300/60 p-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            {buyOpen === "trader" ? (
              ...
              <>
                ...
                <div className="text-center text-xs text-slate-200 mb-3">يوقف نقص جودة السمك بسبب التعفّن للمدة المختارة، والسعر يبقى يتغير طبيعي. تقدر تشتري أكثر من مرة والوقت يتراكم — بحد أقصى 72 ساعة تجميد في نفس الوقت.</div>
                ...
                {freezeActive && <div className="text-center text-[11px] text-cyan-200 mt-2">التجميد فعّال — الشراء يمدد الوقت الحالي (الحد الأقصى 72 ساعة)</div>}
