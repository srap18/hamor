...
function Index() {
  ...
  const getCrewBonuses = (ship: Ship) => {
    ...
    // Guide must be explicitly assigned to a ship — owning a guide is not
    // enough. Each ship needs its own guide crew slot to allow fish selection.
    return {
  ...
  // Police protection is enforced server-side at mission start (a guarded ship
  // simply can't be raided), so there is no client-side auto-catch anymore.
  // Only the manual "catch" button below stops a raider.
  ...
  const collect = async (shipId: number, e: React.MouseEvent) => {
    ...
    // The server clock is authoritative. Local progress can be stale when a
    // mobile app returns from the background, so it must never cap a catch.
    const { data, error } = await (supabase as any).rpc("collect_fishing_reward", {
      _ship_id: s.dbId,
      _requested_fish_id: requestedFishId,
      _client_progress: null,
    });
    if (error) {
  ...
  return (
    ...
    >
      ...
      {scene.id === "hilal" ? (
        ...
        <>
          {/* Attack on Titan background: fixed tap zones only, no overlay effects */}
        ...
        <>
          {/* SpongeBob background: fixed tap zones over the painted signs */}
          {/* Right building "سوق السفن" → Ship Market */}
          <Link
            to="/ship-market"
            aria-label="سوق السفن"
            className="absolute z-[12] rounded-[40%] active:bg-white/10"
