...
function FramePreview({ frame: f }: { frame: Frame }) {
  ...
  if (f.kind === "name") {
    return (
      <div className={`relative px-4 py-2 rounded-lg text-sm font-extrabold tracking-wide ${f.nameClass ?? ""} ${f.animClass ?? ""}`}>
        اللاعب
      </div>
    );
  }
  if (f.kind === "bubble") {
    return (
      <div className={`relative px-3 py-2 rounded-2xl text-xs max-w-[80%] ${f.bubbleClass ?? ""} ${f.animClass ?? ""}`}>
        مرحباً 👋
      </div>
    );
  }
  ...
  return (
    <div className={`w-[90%] flex items-center gap-2 ${f.profileClass ?? ""} ${f.animClass ?? ""}`}>
