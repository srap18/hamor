    ...
    <div className="space-y-2">
      {items.map((it) => (
        <ItemCard key={it.id} item={it} action={
          <button
            type="button"
            onPointerUp={(e) => { e.preventDefault(); e.stopPropagation(); console.log("[equip-tap]", it.id); onEquip(it.id); }}
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); console.log("[equip-click]", it.id); onEquip(it.id); }}
