...
function InventoryPage() {
  ...
  return (
    <div className="fixed inset-0 overflow-y-auto text-foreground" dir="rtl" style={{
      ...
      <header className="sticky top-0 z-20 glass-hud border-b border-accent/30 px-3 pb-3 flex items-center gap-3" style={{ paddingTop: "max(1.75rem, calc(var(--sa-top) + 1.25rem))" }}>
      ...
      <div className="p-3 pb-6">
        ...
        {!loading && tab === "shield" && (() => {
          ...
          const useShield = async (id: string) => {
            ...
            if (error) {
              ...
              if (m.includes("not_enough")) toast.error("لا تملك هذا الدرع");
                ...
                if (!ok) return;
                const { data: skipData, error: skipErr } = await supabase.rpc(
                  "skip_shield_type_cooldown" as never,
                  { _item_id: id } as never
                );
                if (skipErr) {
                ...
                const { error: err2 } = await supabase.rpc(
                  "use_shield_from_inventory" as never,
                  { _item_id: id } as never
                );
                if (err2) {
          ...
          const ANTIS = [
            ...
            { id: "anti_kraken", name: "مضاد قنبلة الكراكن", emoji: "🐙", pct: 100, desc: "يمنع نهب مخزن سمكك" },
          ];
          return (
            <>
            <div className="grid grid-cols-2 gap-2">
