...
function PlayerPage() {
  ...
  const fireWeapon = async (weaponId: string) => {
    ...
    if (firstErr) {
      ...
      if (m.includes("attacker needs fishing ship")) { sound.play("error"); flash("🎣 لازم سفينة وحدة على الأقل تكون في البحر تصيد (وغير مدمّرة) قبل الهجوم"); setBusy(false); return; }
